...
export function CourseProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            const saved = localStorage.getItem("intune-completedLessons");
            if (saved) {
                setCompletedLessons(new Set(JSON.parse(saved)));
            }
            const savedTrainerMode = localStorage.getItem("intune-trainerMode");
            if (savedTrainerMode) {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && completedLessons.size > 0) {
            localStorage.setItem("intune-completedLessons", JSON.stringify([...completedLessons]));
        }
    }, [completedLessons]);
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            localStorage.setItem("intune-trainerMode", JSON.stringify(trainerMode));
        }
    }, [trainerMode]);
